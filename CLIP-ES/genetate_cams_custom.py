...
from clip_text import class_names, new_class_names_coco, BACKGROUND_CATEGORY_CUSTOM, class_names_custom
import argparse
...
def perform(process_id, dataset_list, args, model, bg_text_features, fg_text_features, cam, split_label_list):
    ...
    for im_idx, im in enumerate(tqdm(databin)):
        ...
        for lid in label_id_list:
            label_list.append(class_names_custom[int(lid)])
        ...
        for image in ms_imgs:
            ...
            for idx, label in enumerate(label_list):
                keys.append(class_names_custom.index(label))
        ...
        np.save(os.path.join(args.cam_out_dir, im.replace('png', 'npy')),
                {"keys": keys.numpy(),
...
if __name__ == "__main__":
    ...
    train_list = [x[0] + '.png' for x in file_list]  # [2000:]
    ...
    bg_text_features = zeroshot_classifier(BACKGROUND_CATEGORY_CUSTOM, ['a clean origami {}.'],
                                           model)  # ['a rendering of a weird {}.'], model)
    fg_text_features = zeroshot_classifier(class_names_custom, ['a clean origami {}.'],
                                           model)  # ['a rendering of a weird {}.'], model)
    ...
    if args.num_workers == 1:
        ...
        multiprocessing.spawn(perform, nprocs=args.num_workers,
                              args=(
                              dataset_list, args, model, bg_text_features, fg_text_features, cam, split_label_list))
